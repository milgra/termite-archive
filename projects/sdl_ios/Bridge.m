...
- ( void ) validateProductIdentifiers
{
    ...
    NSArray* productIds = @[ @"termitedonationsmall" , @"termitedonationmedium" , @"termitedonationnormal" ];
           ...
           didReceiveResponse : ( SKProductsResponse* ) response
{
    ...
    for ( SKProduct* product in products )
    {
        ...
        if ( [ product.productIdentifier isEqualToString : @"termitedonationsmall" ] )
        {
            productA = product;
            const char* bytes = [ formattedPrice UTF8String ];
            strcpy( defaults.prices[ 0 ], bytes );
        }
        if ( [ product.productIdentifier isEqualToString : @"termitedonationmedium" ] )
        {
            productB = product;
            const char* bytes = [ formattedPrice UTF8String ];
            strcpy( defaults.prices[ 1 ], bytes );
        }
        if ( [ product.productIdentifier isEqualToString : @"termitedonationnormal" ] )
        {
            productC = product;
            const char* bytes = [ formattedPrice UTF8String ];
