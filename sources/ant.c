    ...
    void ant_free( void* pointer )
    {
        ...
        RPL( ant->food_wanted , NULL );
        RPL( ant->food_taken , NULL );
        RPL( ant->path , NULL );
        RPL( ant->block , NULL );
        RPL( ant->model , NULL );
        RPL( ant->enemy , NULL );
    ...
    ant_t* ant_create( 	ant_t* queen ,
          				float x ,
          				float y ,
          				uint32_t color )
    {

        ant_t* ant = mtmem_calloc(	sizeof( ant_t ) ,
            						ant_free );
        ...
        sprintf( 	ant->id ,
					"%i_%i" ,
					( int ) ( x / defaults.grid_size ) ,
					( int ) ( y / defaults.grid_size )  );
                        ...
                        float hit )
    {
        ...
        if ( ant->energy <= 0.0 )
        {
            ...
            else
            {
                ...
                if ( ant->enemy != NULL ) RPL( ant->enemy , NULL );
    ...
    char ant_followtarget(  ant_t* ant ,
                            v2_t pos ,
                            float distance   )
    {
    ...
    void ant_update_search( ant_t* ant ,
    						float distance )
    {
    ...
    void ant_update_movetofood( ant_t* ant ,
    							float distance )
    {
        ...
        {
            char reached = ant_followtarget( 	ant,
            									ant->food_wanted->pos ,
            									distance );

            if ( reached == 1 )
            {
        		food_set_state( ant->food_wanted ,
        						FOOD_STATE_TAKEN );

                RPL( ant->food_taken , ant->food_wanted );
                RPL( ant->food_wanted , NULL );
                ...
                mtvec_t* path = grid_searchqueen( 	ant ,
                									( void** ) &queen );
                ...
                if ( path != NULL )
                {
					...
					else
					{
						...
						mtvec_reset( ant->path );
						mtvec_addinvector( 	ant->path ,
											path );
    ...
    void ant_update_movetoenemy( 	ant_t* ant ,
    								float distance )
    {
    ...
    void ant_update_movetoqueen( 	ant_t* ant ,
    								float distance )
    {
        		...
        		ant->food_taken != NULL )
        {
            ...
            {
            	food_set_model_pos( ant->food_taken ,
            						ant->model->model.x + 5.0 ,
            						ant->model->model.y + 5.0 );
            }
        }
        else
        {
    ...
    void ant_update_walkpath( 	ant_t* ant ,
     							float distance )
    {
        if ( ant->path->length > 0 )
        {
			...
			if ( reached == 1 )
			{
                ...
                mtvec_removeatindex( ant->path , 0 );
			...
			else
			{
				...
				else if ( ant->block != oldblock || ant->target_lost == 1 )
				{
					...
					food_t* food = grid_searchfood( ant->pos ,
													ant );
    ...
    void ant_update_fight_turn( ant_t* ant )
    {
        ...
        ant_takehit( 	ant->enemy ,
        				ant ,
        				( ( float ) ( rand() % 100 ) / 100.0 ) * defaults.max_hit );

    }


    void ant_update_wait( 	ant_t* ant ,
    						uint32_t ticks )
    {
