    ...
    food_t* food_create( 	float x ,
    						float y ,
    						uint32_t color )
    {
		
        food_t* food = mtmem_calloc( 	sizeof( food_t ) ,
        								food_dealloc );
        ...
        voxel_t* model = HEAP( ( ( voxel_t )
        {
            .model = { 	 x ,
            			-y ,
            			defaults.grid_size / 2.0 + defaults.ant_size ,
            			defaults.ant_size ,
            			defaults.ant_size ,
            			defaults.ant_size } ,
    ...
    void food_set_state( 	food_t* food ,
    						enum food_state state )
    {
    ...
    void food_set_model_pos( 	food_t* food ,
    							float x ,
    							float y )
	{
