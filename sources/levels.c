    ...
    char* levels_get( int index )
    {
        ...
        char* s_managed = mtmem_calloc( strlen( levels[ index ] ) + 1, NULL );
    ...
    char* levels_random( )
    {
		...
		char* result = mtmem_calloc( rows * cols * sizeof( char ) + sizeof( char ) , NULL );
		int index = 0;
    ...
    void levels_add_random_ants( uint32_t color , uint32_t count )
    {
		...
		ant_t* queen = ant_create( 	NULL ,
									(float)(rand() % (int)(grid.width)) ,
									(float)(rand() % (int)(grid.height)) ,
									color );
    ...
    void levels_add_random_food_in_rect( uint32_t count ,
    									float x ,
    									float y ,
    									float width ,
    									float height )
    ...
    void levels_add_random_food_in_circle( 	uint32_t count ,
    										float x ,
    										float y ,
    										float radius )
    {
    ...
    mtvec_t* levels_text_to_blocks( char* leveltext , uint32_t* cols , uint32_t* rows )
    {
        
        mtvec_t* gblocks = mtvec_alloc( );
    ...
    void levels_blocks_rotate( mtvec_t* blocks , uint32_t* cols , uint32_t* rows )
    {
        ...
        for ( int col = *cols - 1 ; col > -1 ; col-- )
        {
            for ( int row = 0 ; row < *rows ; row++ )
            {
                ...
                mtvec_add( blocks , block );
            
            }

        }

        mtvec_removeinrange( blocks , 0 , index - 1 );
    ...
    void levels_load( int level )
    {
        ...
        mtvec_t* gblocks = levels_text_to_blocks( leveltext , &cols , &rows );
        ...
        for ( int row = 0 ; row < rows ; row++ )
        {
            ...
            for ( int col = 0 ; col < cols ; col++ )
            {
                ...
                if ( block->wall == 0 )
                {
                    ...
                    {
                        .model = { col * defaults.grid_size ,
                                    -((float)row * defaults.grid_size) ,
                                    defaults.grid_size / 2.0 ,
                                    defaults.grid_size ,
                                    defaults.grid_size ,
                                    defaults.grid_size } ,
                    ...
                    mtvec_add( grid.tilevoxels , voxel );
