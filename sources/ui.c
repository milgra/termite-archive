//  Created by Milan Toth on 2018. 01. 19.
						...
						float pixelsize )
	{
		...
		string = mtcstr_fromcstring( label );

		button = voxelutil_button( 	string ,
									x ,
									y ,
									width ,
									height ,
		...
		mtvec_add( 	ui.buttons ,
				  	button->data[ button->length - 1 ] );

		mtvec_addinvector( 	ui.voxels ,
						  	button );
	...
	void ui_generate_header( )
	{
		...
		itemasize = pixeltext_calcsize( "TERMITE" ,
										29.0 );

		ui_add_button( 	"TERMITE" ,
						itemasize.x / -2.0 - MARGIN ,
						-100.0 ,
						itemasize.x + 2 * MARGIN ,
						itemasize.y + 2 * MARGIN ,
					  	defaults.textcolor_a ,
					  	0 ,
						29.0 );

		itembsize = pixeltext_calcsize( "BY MILAN TOTH" ,
										8.0 );

		ui_add_button( 	"BY MILAN TOTH" ,
						itemasize.x / 2.0 - itembsize.x - MARGIN ,
						-130.0 -itemasize.y ,
						itembsize.x + 2 * MARGIN ,
						itembsize.y + 2 * MARGIN ,
					  	defaults.textcolor_a ,
					  	defaults.buttoncolor_a ,
						8.0 );

        itemcsize = pixeltext_calcsize( TERMITE_VERSION ,
                                        5.0 );
    ...
    void ui_generate_menu( mtvec_t* items )
    {
         			...
         			index++ )
        {
            ...
            label 	= mtstr_bytes( item );

            itemsize = pixeltext_calcsize( 	label ,
											10.0 * defaults.text_scale );

			ui_add_button( 	label ,
							itemsize.x / -2.0 - MARGIN ,
							position.y ,
							itemsize.x + 2 * MARGIN ,
							itemsize.y + 2 * MARGIN ,
						  	defaults.textcolor_a ,
    ...
    void ui_generate_info( 	char** items ,
    						int count )
    {
    ...
    void ui_generate_toolbar( )
    {
		...
		ui_add_button( 	"SPEED" ,
						-( size_spd + size_mnu ) / 2.0 - 2.0 ,
					  	-HTH + size_hth * defaults.text_scale ,
					  	size_spd ,
					  	size_hth ,
					  	defaults.textcolor_a ,
					  	defaults.buttoncolor_a ,
		...
		ui_add_button( 	"MENU" ,
						( size_spd + size_mnu ) / 2.0 - size_mnu + 2.0 ,
					  	-HTH + size_hth * defaults.text_scale,
					  	size_mnu ,
					  	size_hth ,
					  	defaults.textcolor_a ,
    ...
    void ui_generate_level_selector( mtvec_t* voxels ,
    								 mtvec_t* names ,
									 mtvec_t* buttons ,
									 int startindex )
    {
				  ...
				  row++ )
        {
            			...
            			col++ )
            {
				...
				itemsize = pixeltext_calcsize( text ,
											   textsize );
		...
		itemsize = pixeltext_calcsize( "RESET STATE" ,
										10.0 );

		ui_add_button( "RESET STATE" ,
						-itemsize.x / 2.0 - MARGIN ,
						-6.0 * size ,
						itemsize.x + 2 * MARGIN ,
						itemsize.y + 2 * MARGIN ,
    ...
    void ui_updateui( )
    {
        ...
        mtbus_notify( "SCN" , "UPLOADVOXELS" , &data );
    ...
    void ui_updateperspective( )
    {
        ...
        m4_t pers = m4_defaultperspective( 	camera_fov_y ,
    ...
    void ui_updateprojection( )
    {
        ...
        m4_t scale_matrix = m4_defaultscale( 	ui.base_scale ,
        									 	ui.base_scale ,
        									 	ui.base_scale );
        ...
        if ( ui.state != UISTATE_GAME )
        {
        	...
        	trans_matrix = m4_defaulttranslation( 	0.0 ,
													HTH / 2.0 ,
													0.0 );
		...
		ui.mat_proj = m4_multiply( ui.mat_pers ,
								   angle_matrix );

		ui.mat_proj = m4_multiply( ui.mat_proj ,
								   scale_matrix );
    ...
    void ui_generate_main_menu( char* menuitem )
	{
        ...
        mtvec_t* mainitems = mtvec_alloc( );

        if ( menuitem == NULL )
        {
            mtvec_add( mainitems , mtstr_frombytes( "PLAY" ) );
            mtvec_add( mainitems , mtstr_frombytes( "OPTIONS" ) );
            if ( defaults.prices_arrived == 1 ) mtvec_add( mainitems , mtstr_frombytes( "DONATE" ) );
            #if !defined( IOS ) && !defined( ANDROID )
            mtvec_add( mainitems , mtstr_frombytes( "EXIT" ) );
            #endif
        }
        else if ( strcmp( menuitem , "PLAY" ) == 0 )
        {
            mtvec_add( mainitems , mtstr_frombytes( "TOURNAMENT" ) );
            mtvec_add( mainitems , mtstr_frombytes( "RANDOM" ) );
            mtvec_add( mainitems , mtstr_frombytes( "BACK" ) );
        }
        else if ( strcmp( menuitem , "OPTIONS" ) == 0 )
        {
            mtvec_add( mainitems , mtstr_frombytes( "2D/3D" ) );
            mtvec_add( mainitems , mtstr_frombytes( "COLOR" ) );
            mtvec_add( mainitems , mtstr_frombytes( "EFFECTS" ) );
            #if !defined( IOS ) && !defined( ANDROID )
            mtvec_add( mainitems , mtstr_frombytes( "FULLSCREEN" ) );
            #endif
            mtvec_add( mainitems , mtstr_frombytes( "FEEDBACK" ) );
            mtvec_add( mainitems , mtstr_frombytes( "BACK" ) );
        }
        else if ( strcmp( menuitem , "DONATE" ) == 0 )
        {
            ...
            {
                mtvec_add( mainitems , mtstr_frombytes( defaults.prices[ index ] ) );
            }
            mtvec_add( mainitems , mtstr_frombytes( "BACK" ) );
        }
	...
	void ui_generate_grid( int from )
	{
		...
		ui_generate_level_selector( ui.voxels ,
									ui.names ,
									ui.buttons ,
									from );
    ...
    void ui_changestate( uistate state ,
						 char* menuitem )
    {

        mtvec_reset( ui.voxels );
        mtvec_reset( ui.names );
        mtvec_reset( ui.buttons );
    ...
    void ui_handle_mainbutton( char* name )
    {
		...
		if 		( strcmp( name , "BY MILAN TOTH"	) == 0 ) mtbus_notify( "UI" , "HOMEPAGE" , NULL );
		...
		else if ( strcmp( name , "RANDOM" 			) == 0 )
		{
			...
			mtbus_notify( "UI", "LOAD" , &defaults.current_level );

		}
		else if ( strcmp( name , "BACK" 			) == 0 ) ui_changestate( UISTATE_MAIN , NULL );

		else if ( strcmp( name , "FEEDBACK" 		) == 0 ) mtbus_notify( "UI" , "FEEDBACK" , NULL );
		else if ( strcmp( name , "FULLSCREEN" 		) == 0 ) mtbus_notify( "UI" , "FULLSCREEN" , NULL );
		...
		else if ( strcmp( name , "EFFECTS" ) == 0 )
		{
			...
			mtbus_notify( "UI", "EFFECTS_LEVEL" , NULL );
    					 ...
    					 v2_t local )
    {
        ...
        if ( strcmp( name , "SPEED" ) == 0 )
        {
            ...
            mtbus_notify( "UI" , "SETSPEED" , &ratio );
    ...
    char* ui_getpushedbutton( void* data ,
							  v2_t* local )
    {

		v2_t screenpt = *( v2_t* ) data;

        mtvec_t* items = ui.buttons;
        mtvec_t* names = ui.names;

        v4_t front = v4_init( screenpt.x ,
							  ui.screen.y - screenpt.y ,
							  -1.0 ,
							  0.0 );

        v4_t back  = v4_init( screenpt.x ,
							  ui.screen.y - screenpt.y ,
							  0.0 ,
							  0.0 );

        v3_t frontprj = m4_screen_to_world_coords( 	ui.mat_proj ,
        											front ,
        											ui.screen.x ,
        											ui.screen.y );

        v3_t backprj = m4_screen_to_world_coords( 	ui.mat_proj ,
        											back ,
        											ui.screen.x ,
        											ui.screen.y );
    ...
    void ui_onmessage( const char* name ,
    				   void* data )
    {
    ...
    void ui_init( )
    {
        ...
        mtbus_subscribe( "CTL" , ui_onmessage );
        mtbus_subscribe( "SCN" , ui_onmessage );

        ui.buttons = mtvec_alloc();
        ui.voxels = mtvec_alloc();
        ui.names = mtvec_alloc();
	...
	void ui_free( )
	{
		...
		mtbus_unsubscribe( "CTL" , ui_onmessage );
		mtbus_unsubscribe( "SCN" , ui_onmessage );
