    ...
    void scene_init( )
    {
        ...
        scn.sparks = mtvec_alloc( );
        scn.clouds = mtvec_alloc( );
        scn.shards = mtvec_alloc( );
        scn.voxels = mtvec_alloc( );
        scn.litter = mtvec_alloc( );
        scn.births = mtvec_alloc( );
    ...
    void scene_convert_voxels_to_shards( mtvec_t* voxels )
    {
					...
					index++ )
		{
			...
			v3_t direction = { 	-1.0 + ( float ) ( rand( ) % 200 ) / 100.0 ,
								-1.0 + ( float ) ( rand( ) % 200 ) / 100.0 ,
								-1.0 + ( float ) ( rand( ) % 200 ) / 100.0 };

			particle_t* shard = particle_create( voxel , direction );

			mtvec_add( scn.voxels , voxel );
			mtvec_add( scn.shards , shard );
	...
	void scene_createshards( )
	{
		...
		mtvec_t* temp = mtvec_alloc( );
		mtvec_addinvector( temp , scn.voxels );
		mtvec_reset( scn.voxels );
    ...
    void scene_create_spark( ant_t* ant )
    {
		...
		v3_t direction = { 	-0.5 + ( float )( rand( ) % 100 ) / 100.0 ,
							-0.5 + ( float )( rand( ) % 100 ) / 100.0 ,
							 4.0 + ( float )( rand( ) % 20 ) / 10.0 };
		...
		mtvec_add( 	scn.sparks , spark );
		mtvec_add( 	scn.voxels , spark->model );
    ...
    void scene_update_sparks( float distance )
    {
					...
					index++ )
		{
			...
			if ( spark->finished == 1 ) mtvec_add( scn.litter , spark );
		...
		if ( scn.litter->length > 0 )
		{
						...
						index++ )
			{
				...
				mtvec_remove( scn.sparks , spark );
				mtvec_remove( scn.voxels , spark->model );
			}

			mtvec_reset( scn.litter );
		}
    ...
    void scene_createclouds( )
    {
        ...
        for ( int index = 0 ; index < 20 ; index++ )
        {
            ...
            voxel_t* model = HEAP( ( ( voxel_t )
            {
                .model = { 	( float )( rand( ) % 1500 ) ,
                			450.0 -( float )( rand( ) % 1500 ) ,
						   -50.0 - index ,
                			200 + ( float )( rand( ) % 500 ) ,
                			50 + ( float )( rand( ) % 150 ) ,
                			50.0 } ,
            ...
            v3_t direction = { 	1.0 + ( float )( rand( ) % 10 ) / 10.0 ,
            					0.0 ,
            					0.0 };

            particle_t* cloud = particle_create( model , direction );

            mtvec_add( scn.clouds , cloud );
            mtvec_add( scn.voxels , cloud->model );
    ...
    void scene_create_ant( ant_t* ant )
    {
		...
		ant_t* newant = ant_create( ant ,
									ant->pos.x ,
									ant->pos.y ,
									ant->color );

		mtvec_add( scn.births , newant );
    ...
    void scene_load( void* data )
    {
        ...
        mtvec_add( scn.voxels , control.vox_rect );
    ...
    void scene_onmessage( 	const char* name ,
    						void* data )
    {
